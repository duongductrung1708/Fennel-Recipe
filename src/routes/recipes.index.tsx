...
import { categories, cuisines, dietaryTags, ingredientIndex, recipes } from "@/data/recipes";
import {
  Sheet,
  SheetContent,
  SheetDescription,
...
function RecipesPage() {
  ...
  const suggestions = useMemo<Suggestion[]>(() => {
    ...
    const rec: Suggestion[] = recipes
      ...
      .slice(0, 4)
      .map((r) => ({ type: "recipe", value: r.title, label: r.title, slug: r.slug }));
  ...
  const filtered = useMemo(() => {
    ...
    return recipes.filter((r) => {
      ...
      if (diets.length > 0 && !diets.every((d) => r.dietary.includes(d as never))) return false;
      if (!q) return true;
      const haystack = [r.title, r.tagline, r.cuisine, ...r.ingredients, ...r.dietary]
        .join(" ")
        .toLowerCase();
  ...
  const hasFilters = query !== "" || cuisine !== "All" || category !== "All" || diets.length > 0;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      <header className="mb-10">
        <p className="text-primary uppercase tracking-[0.25em] text-xs mb-3">The collection</p>
        <h1 className="font-display text-5xl md:text-6xl">All recipes</h1>
        <p className="mt-4 text-muted-foreground max-w-xl">
          Search by ingredient or name. Filter by cuisine or course. Click a card to start cooking.
        </p>
      </header>
        ...
        <section>
          <div className="mb-5 flex items-center justify-between gap-3 lg:hidden">
            <Sheet>
              ...
              <SheetContent side="left" className="w-[92vw] max-w-sm overflow-auto">
                <SheetHeader className="mb-4">
                  <SheetTitle>Filters</SheetTitle>
                  <SheetDescription className="sr-only">
                    Search and filter recipes by cuisine, course, and dietary preferences.
                  </SheetDescription>
                </SheetHeader>
                <RecipesFilters
